...
ref = np.zeros((4, 1100,1100)) #initializing references and masks arrays. Change these sizes according to the number of references and size of images
mask_disk = np.ones((4,1100,1100))
masks_fom = np.ones((4,1100,1100))
...
for x in sorted(os.listdir(directory)):
    ...
    counter2 += 1
    if counter2 == 25: #this block medians/stacks all of the references based on the visit. Change the counter value based on the number of images per visit (28 img are used for the 2012 and 2013 data)
        counter2 = 0
